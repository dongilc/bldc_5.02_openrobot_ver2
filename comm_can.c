...
//openrobot
void comm_can_set_baud_custom(uint8_t index) {
/*
	// original index
	CAN_BAUD_125K = 0,
	CAN_BAUD_250K = 1,
...
*/
	//openrobot
	if(index == 2) {
		set_timing(5, 9, 2);	// 500k bps
		commands_printf("CAN Baudrate Set as 500Kbps");
	} else if(index == 3) {
		set_timing(2, 9, 2);	// 1M bps
		commands_printf("CAN Baudrate Set as 1Mbps");
	} else if(index == 10) {
		set_timing(1, 3, 1); 	// 3M bps,  set_timing(brp, ts1, ts2)
		commands_printf("CAN Baudrate Set as 3Mbps");
	} 		
}
	...
	for(;;) {
		...
		while ((rxmsg_tmp = comm_can_get_rx_frame()) != 0) {
			...
			if (rxmsg.IDE == CAN_IDE_EXT) {
				...
				// kitech, standard id process. 2022-01-24.
				decode_msg(rxmsg.SID, rxmsg.data8, rxmsg.DLC, false);
			}
		}
	}
}
...
//openrobot
			...
			if (stat_tmp->id == id || stat_tmp->id == -1) {
				...
				float pos = (float)(res_temp/100.);	//211111
				ind += 3;
				...
				// motor_temp - C
				res_temp = (eid >> 16) & 0xFFFF;
				float motor_temp = (float)(res_temp/10. - 273.0);
				stat_tmp_4->id = id;
				stat_tmp_4->temp_motor = motor_temp;
				break;
			}
#else
			can_status_msg *stat_tmp = &stat_msgs[i];
...
//openrobot
#if defined(HW60_IS_VESCULAR) || defined(HW60_IS_VESCUINO) 
	int32_t pos_rad = (int32_t)(mcpwm_foc_get_pos_accum()*M_PI/180.*100.0);	//211111
	int32_t vel_rps = (int32_t)(mcpwm_foc_get_rps()*10000.0);
	int16_t curr = (int16_t)(mc_interface_get_tot_current_filtered()*100.0);

	// send temp_motor using eid
	int16_t temp_motor = (int16_t)( (mc_interface_temp_motor_filtered() + 273.0) * 10.0);
	...
	// this is eid version sending temp_motor in eid
	//comm_can_transmit_eid_replace(id | ((uint32_t)CAN_PACKET_STATUS << 8) | (temp_motor << 16),
	//		buffer, send_index, replace);

	// kitech 2022-01-24. sending id as standard id
	comm_can_transmit_sid(id | ((uint32_t)CAN_PACKET_STATUS << 4), buffer, send_index);		
#else
